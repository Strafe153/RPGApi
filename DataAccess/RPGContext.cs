...
public class RPGContext
{
	...
	private const string DatabaseName = "rpg_api_db";
	...
	private void EnsureDatabase()
	{
		...
		var queryParams = new { DBName = DatabaseName };
	...
	{
		var (hash, salt) = PasswordHelper.GeneratePasswordHashAndSalt(_adminOptions.Password);
		var tableQuery = @"
	...
	private void InitializeCharacters(IDbTransaction transaction)
	{
		...
		var indexQuery = @"
            CREATE INDEX IF NOT EXISTS ""IX_Characters_PlayerId""
	...
	{
		var tableQuery = @"
            CREATE TABLE IF NOT EXISTS public.""CharacterWeapons""
            (
		...
		var indexQuery = @"
            CREATE INDEX IF NOT EXISTS ""IX_CharacterWeapons_WeaponId""
	...
	{
		var tableQuery = @"
            CREATE TABLE IF NOT EXISTS public.""CharacterSpells""
            (
                ""CharacterId"" integer NOT NULL,
		...
		var indexQuery = @"
            CREATE INDEX IF NOT EXISTS ""IX_CharacterSpells_SpellId""
	...
	{
		var tableQuery = @"
            CREATE TABLE IF NOT EXISTS public.""CharacterMounts""
            (
                ""CharacterId"" integer NOT NULL,
		...
		var indexQuery = @"
            CREATE INDEX IF NOT EXISTS ""IX_CharacterMounts_MountId""
